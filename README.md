...
* __(NEW!) View .md files in a markdown viewer__: Press the A button on a file and select `Show in MD ToC Viewer` (only shows up for actual .md files). Select sections with dpad and press A to confirm selection and read that section. If you put .md files in vram at compilation time, in `HOME` -> `More...` -> `Show ReadME` there will be soft keyboard to enter .md alternative filenames
...
* __(NEW!) View .md files via a neat menu__: Press the HOME button, select `More...` -> `Scripts...`. Any .md file you put into `0:/gm9/markdown/` (subdirs included) will be found here.
