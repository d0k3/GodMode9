...
static int InitSaveWearLeveling(CartData *cdata) {
    ...
    savectx.logical_sectors = num_physical_sectors - 1;
    savectx.logical_size = (num_physical_sectors - 1) * 0x1000;
    ...
    if (num_physical_sectors > 128) {
        ...
        savectx.blockmap_size = sizeof(SaveBlockmapHeader) + savectx.logical_sectors * sizeof(SaveBlockmapEntryV1) + sizeof(u16);
        savectx.type = CARD_SAVE_WEAR_LEVELING_V1;
    }
    
    // TODO: add CMAC verification ability for DISA 
    
    if (ReadCartSave(savectx.blockmap, 0, savectx.blockmap_size, cdata) != 0)
        return 1;
    
    u32 journal_size = 0x1000 - savectx.blockmap_size;
    savectx.num_journal_entries = journal_size / sizeof(SaveJournalEntry);
    
    if (ReadCartSave((u8 *)savectx.journal, savectx.blockmap_size, journal_size, cdata) != 0)
        return 1;
    ...
    for (; journal_idx < savectx.num_journal_entries; journal_idx++) {
        ...
        if (ent->main.dst_phys_sec) {
            ...
            if (ent->main.src_virt_sec >= savectx.logical_sectors)
                break;
...
int InitCtrCardSave(CartData *cdata) {
    ...
    if (InitSaveWearLeveling(cdata) != 0)
        return 1;
